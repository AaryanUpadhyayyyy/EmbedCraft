...
class NinjaCallbackManager {
  ...
  ///   NinjaLog.d('AppNinja', event.action);
  /// });
  /// ```
  ...
  static void registerListener(NinjaCallbackListener listener) {
    if (!_listeners.contains(listener)) {
      ...
      NinjaLog.d('CallbackManager', '🔔 NinjaCallbackManager: Listener registered (${_listeners.length} total)');
    }
  }
  ...
  static void unregisterListener(NinjaCallbackListener listener) {
    if (_listeners.remove(listener)) {
      NinjaLog.d('CallbackManager', '🔕 NinjaCallbackManager: Listener unregistered (${_listeners.length} remaining)');
    }
  }
  ...
  static void clearAllListeners() {
    _listeners.clear();
    NinjaLog.d('CallbackManager', '🧹 NinjaCallbackManager: All listeners cleared');
  }
  ...
  static void dispatchEvent(NinjaCallbackData event) {
    NinjaLog.d('CallbackManager', '📢 NinjaCallbackManager: Dispatching ${event.action} to ${_listeners.length} listener(s)');
    ...
    for (final listener in _listeners.toList()) {
      try {
        ...
        ninjaSdkLogError(e, st, 'NinjaCallbackManager.listener');
      }
    }
    ...
    try {
      ...
      ninjaSdkLogError(e, st, 'NinjaCallbackManager stream');
    }
  }
  ...
  static void dispatchComponentCtaClick({
    ...
    // Map CLICK_TYPE to required unique values
    String mappedClickType = clickType;
    if (clickType == 'callback' || clickType == 'custom') mappedClickType = 'CB';
    else if (clickType == 'deeplink') mappedClickType = 'deep_link';
    else if (clickType == 'link' || clickType == 'external_url' || clickType == 'open_link') mappedClickType = 'external';
    else if (clickType == 'close') mappedClickType = 'dismiss';

    // Extract target, checking eventName for dashboard 'custom' action parity
    String finalTarget = target ?? additionalData?['eventName']?.toString() ?? '';

    // Clean additional data to prevent polluting core keys
    final safeData = Map<String, dynamic>.from(additionalData ?? {});
    safeData.remove('type');
    safeData.remove('value');
    safeData.remove('url');
    safeData.remove('action');
    safeData.remove('eventName');
  ...
  /// Dispatch story slide opened event (UI)
  static void dispatchSlideOpen({
    required String campaignId,
  ...
  /// Dispatch story slide dismissed event (UI)
  static void dispatchSlideDismiss({
    required String campaignId,
    required int slideIndex,
    Map<String, dynamic>? additionalData,
  }) {
    dispatchEvent(NinjaCallbackData(
      type: 'UI',
      action: NINJA_SLIDE_DISMISS,
      method: 'slide_dismiss',
      data: {
        'CAMPAIGN_ID': campaignId,
        'SLIDE_INDEX': slideIndex,
        ...?additionalData,
  ...
  /// Dispatch spin wheel actively spun event (UI)
  static void dispatchSpinWheelSpun({
    required String campaignId,
    Map<String, dynamic>? additionalData,
  }) {
    dispatchEvent(NinjaCallbackData(
      type: 'UI',
      action: NINJA_SPIN_WHEEL_SPUN,
      method: 'spin',
      data: {
        'CAMPAIGN_ID': campaignId,
        ...?additionalData,
